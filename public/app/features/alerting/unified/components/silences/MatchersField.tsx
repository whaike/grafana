...
const MatchersField: FC<Props> = ({ className }) => {
  ...
  const matcherOptions = Object.values(MatcherOperatorOptions).map((value) => ({ label: value, value }));
      ...
      <Field label="Matching labels" required>
        <div>
          ...
          <Button
            ...
            variant="secondary"
            onClick={() => {
              const newMatcher = { name: '', value: '', operator: MatcherOperatorOptions.equal };
              append(newMatcher);
            }}
          >
            Add matcher
